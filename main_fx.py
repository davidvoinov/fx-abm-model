"""
main_fx.py — Interactive demo of the multi-venue FX ABM.

Run:
    python3 main_fx.py                          # default scenario
    python3 main_fx.py --preset heavy_amm       # 60% AMM flow
    python3 main_fx.py --preset clob_only       # no AMM pools
    python3 main_fx.py --amm-share 40 --shock-iter 250 --shock-pct -15

All configurable parameters are exposed as CLI flags.
Use  python3 main_fx.py --help  for the full list.
"""
...
# ═══════════════════════════════════════════════════════════════════════
#  PRESETS — named parameter bundles for common scenarios
# ═══════════════════════════════════════════════════════════════════════

PRESETS = {
    # balanced mixed market — all defaults
    "default": dict(),

    # CLOB only, no AMM pools
    "clob_only": dict(
        enable_amm=0,
        amm_share_pct=0,
    ),

    # AMM only — ShadowCLOB provides synthetic reference; exogenous GBM S_t
    "amm_only": dict(
        enable_clob_mm=0,
        clob_liq=0.1,
        amm_share_pct=95,
    ...
    # 60 % of flow goes to AMM, double AMM reserves
    "heavy_amm": dict(
        amm_share_pct=60,
        amm_liq=2.0,
    ),

    # 90 % CLOB, double CLOB depth
    "heavy_clob": dict(
        amm_share_pct=10,
        clob_liq=2.0,
    ),

    # prolonged & severe stress regime
    "stress_test": dict(
        stress_start=150,
        stress_end=400,
        sigma_high=0.08,
    ...
    # no gradual stress, only a one-time price shock
    "shock_only": dict(
        stress_start=-1,
        stress_end=-1,
        shock_iter=250,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # ── FX Liquidity Takers (multi-venue) ────────────────────────────
    g = p.add_argument_group(
        "FX liquidity takers",
...
def build_sim(args: argparse.Namespace) -> Simulator:
    """Construct Simulator from parsed CLI arguments."""
...
def print_config(args: argparse.Namespace):
    """Pretty-print the full configuration before simulation."""
    W = 65
    print("\n" + "=" * W)
...
def print_summary(sim: Simulator):
    """Print key metrics organised by hypothesis."""
    logger = sim.logger
...
def main():
    ...
    # Apply preset first, then CLI flags override
    if args.preset:
